...
from padp.dataset.base_dataset import BaseImageDataset, LinearNormalizer
from padp.model.common.normalizer import LinearNormalizer, SingleFieldLinearNormalizer
from padp.model.common.rotation_transformer import RotationTransformer
from padp.common.imagecodecs_numcodecs import register_codecs, Jpeg2k
from padp.common.replay_buffer import ReplayBuffer
from padp.common.sampler import get_val_mask
from padp.common.normalize_util import (
...
class RobomimicReplayImageDataset(BaseImageDataset):
    ...
    def __init__(
        ...
        self.n_action_steps = int(n_action_steps) if n_action_steps is not None else self.horizon
...
# 1+2 步：转换阶段完成“物理化填充 + pos_info + episode_map”
        replay_buffer = None
        if use_cache:
            rotation_tag = str(rotation_rep).replace('/', '_')
            shape_tag = self._shape_cache_tag(shape_meta)
            # 移除 .zip 后缀，改为普通的 .zarr 文件夹路径
            cache_zarr_path = (
                f'{dataset_path}'
                + f'.n{n_demo}.h{self.horizon}.obs{self.n_obs_steps}.act{self.n_action_steps}'
                + f'.abs{int(abs_action)}.rot{rotation_tag}.shape{shape_tag}'
            ...
            with FileLock(cache_lock_path):
                if not os.path.exists(cache_zarr_path):
                    try:
                        print('Cache folder does not exist. Creating (RTV8)!')
                        # 直接创建 DirectoryStore 写入磁盘文件夹
                        ...
                        print('Successfully created RTv8 DirectoryStore.')
                    except Exception as e:
                        if os.path.exists(cache_zarr_path):
                            shutil.rmtree(cache_zarr_path)
                        raise e
                else:
                    print('Loading RTV8 ReplayBuffer directly from Disk folder.')
                    # 直接挂载磁盘文件夹，不再 copy 到 MemoryStore，释放 100G+ 内存
                    store = zarr.DirectoryStore(cache_zarr_path)
                    replay_buffer = ReplayBuffer(zarr.group(store))
                    print('Loaded!')
        else:
            replay_buffer = _convert_robomimic_to_replay_v8(
        ...
        if mapping_dir is None:
            mapping_dir = (
                ...
                + f'.rtdp_v8_maps.n{self.n_demo}.h{self.horizon}.obs{self.n_obs_steps}.act{self.n_action_steps}'
                + f'.b{self.batch_size}'
            )
        ...
        if build_epoch_mappings and build_epoch_mappings > 0:
            for epk in range(build_epoch_mappings):
                ...
                if not os.path.exists(path):  # 添加映射表存在性检查​，避免重复构建已存在的映射表
                    ...
                    print(f"build_epoch_mappings > 0, Mapping for epoch {epk} already exists. Skipping.")
            ...
            for epk in range(self.num_epochs):
                ...
                if not os.path.exists(path):
                    ...
                    print(f"self.num_epochs > 0, Mapping for epoch {epk} already exists. Skipping.")
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        ...
        # ==============================================================================
        # [TODO: 性能优化暂缓] GPU 图像归一化延迟计算 (uint8 -> float32)
        # ==============================================================================
        # 【暂缓原因】：当前正在进行核心消融实验。为了保证完全的控制变量，避免任何
        # 数据流、内存分配或底层张量操作的时序变化干扰最终的成功率，暂时保持原样。
        # 
        # 【优化方案】：将 Dataset 中的 `.astype(np.float32) / 255.0` 移除，
        # 改在 Workspace (GPU) 的 `batch.to(device)` 后执行。
        # 预期收益：大幅降低 PCIe 带宽压力，加速 DataLoader。
        #
        # 【执行时机】：本轮消融实验（[填写你的实验代号/日期]）全部结束后，
        # 再解开以下注释并进行性能对照测试。
        # ==============================================================================
        ...
        for key in self.rgb_keys:
            ...
            # 从“CPU Dataset 里做”改成“GPU 上做”。可能会加快
    ...
    def _validate_window_num(self, episode_ids: np.ndarray):
        ...
        if len(bad) > 0:
            ...
            raise RuntimeError(
                f'window_num_1 != window_num_2 for some episodes! '
                f'Current horizon={self.horizon}. '
                f'If you changed horizon / abs_action / rotation_rep, delete the old RTV8 cache and rebuild it.'
            )

    def _shape_cache_tag(self, shape_meta: dict) -> str:
        plain_shape_meta = OmegaConf.to_container(shape_meta, resolve=True)
        payload = {
            'action': plain_shape_meta.get('action', {}),
            'obs': {
                key: {
                    'type': value.get('type', 'low_dim'),
                    'shape': value.get('shape', None),
                }
                for key, value in sorted(plain_shape_meta.get('obs', {}).items())
            },
        }
...
def _convert_robomimic_to_replay_v8(
    ...
    with h5py.File(dataset_path, 'r') as file:
        ...
        for key in rgb_keys:
            ...
            # 使用业界高效的 Blosc-LZ4 压缩，显著降低 CPU 解压负担
            this_compressor = Blosc(cname='lz4', clevel=5, shuffle=Blosc.NOSHUFFLE)
            img_arrs[key] = data_group.require_dataset(
                name=key,
                shape=(total_padded, h_img, w_img, c),
                # 增大 chunks 到 16，提升 NVMe 硬盘的随机读取效能
                chunks=(16, h_img, w_img, c),
        ...
        for e in tqdm(range(n_demo), desc='RTV8: materialize padded episodes'):
            ...
            for key in rgb_keys:
                arr = demo['obs'][key]
                # 创建填充后的图像数组
                padded_arr = np.empty((real_len_new, *arr.shape[1:]), dtype=arr.dtype)
                
                # 前 pad_b
                if pad_b > 0:
                    padded_arr[:pad_b] = arr[0]
                
                # 中段原始
                padded_arr[pad_b:pad_b+real_len] = arr[:real_len]
                
                # 尾 pad_a
                if pad_a > 0:
                    padded_arr[pad_b+real_len:] = arr[-1]
                
                # 批量写入
                img_arrs[key][ofs:ofs+real_len_new] = padded_arr
    ...
    # 创建ReplayBuffer（直接关联到刚才写入的磁盘 root 组）
