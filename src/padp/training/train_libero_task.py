...
from padp.data.openpi_libero_task_loader import fit_task_padp_normalizer
from padp.data.openpi_libero_task_loader import load_padp_normalizer
from padp.data.openpi_libero_task_loader import save_padp_normalizer


def parse_args() -> argparse.Namespace:
    repo_root = Path(__file__).resolve().parents[3]
    parser = argparse.ArgumentParser(description="Train a task-conditioned PADP-VA policy on LIBERO data.")
...
def main() -> None:
    ...
    print("Instantiating task-conditioned PADP policy...")
    ...
    print(f"Starting task-conditioned PADP LIBERO training for {max_steps} steps on {device}...")
    for step in range(max_steps):
        ...
        if args.backward_only:
            print(f"task backward-only smoke ok, step={step}, loss={float(batch_loss.detach().cpu())}")
            return
    ...
    print(f"Task-conditioned PADP LIBERO train finished. Output dir: {output_dir}")
...
def prepare_normalizer(cfg: Any, normalizer_path: Path, args: argparse.Namespace):
    if normalizer_path.exists():
        print(f"Loading PADP task normalizer from: {normalizer_path}")
        return load_padp_normalizer(normalizer_path)

    if not args.fit_normalizer_if_missing:
        raise FileNotFoundError(
            f"PADP task normalizer not found: {normalizer_path}. "
            "Run padp.training.compute_norm_stats_for_padp_task first."
        )

    print(f"PADP task normalizer not found; fitting from {args.normalizer_batches} batches...")
    dataset = make_dataset(cfg, num_batches=args.normalizer_batches)
    normalizer = fit_task_padp_normalizer(
        iter(dataset),
        num_batches=args.normalizer_batches,
        task_obs_key=str(cfg.task_condition.obs_key),
    )
    save_padp_normalizer(
        ...
        normalizer_path,
        metadata={
            "source": "train_libero_task auto-fit",
    ...
    print(f"Saved auto-fit PADP task normalizer to: {normalizer_path}")
    return normalizer
