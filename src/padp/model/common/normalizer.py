...
from padp.model.common.dict_of_tensor_mixin import DictOfTensorMixin
...
def _normalize(x, params, forward=True):
    ...
    # 快路径：训练小批次通常已经和归一化参数位于同一块显卡且数据类型一致。
    # 这种常见情况下跳过 .to()，避免在高频路径里发起无意义的拷贝或类型转换。
    if x.device != scale.device or x.dtype != scale.dtype:
