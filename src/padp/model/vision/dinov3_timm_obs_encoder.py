...
from padp.model.vision.choice_randomizer import RandomChoice
...
class TimmObsEncoder(ModuleAttrMixin):
    def __init__(self,
        ...
        """
        Expects flattened PADP observations.
        ...
        for key, attr in obs_shape_meta.items():
            ...
            type = attr.get('type', 'low_dim')
            if type == 'rgb':
                # PADP adapter provides image shape as (C, H, W), for example (3, 224, 224).
                if image_shape is None or image_shape == shape:
                    image_shape = shape
                else:   
                    if len(shape) == 3 and len(image_shape) == 3:
                        if shape[1:] != image_shape[1:]:
                            logger.warning(f"Different image shapes: {shape} vs {image_shape}")
        
        if image_shape is not None and len(image_shape) == 3:
            _, H, W = image_shape
            img_size = (H, W)  # (224, 224)
        else:
            logger.warning("No RGB input found, using default image size 224x224")
            H = W = 224
            img_size = (H, W)
        ...
        if transforms is not None and not isinstance(transforms[0], torch.nn.Module):
            ...
            transforms = [
                torchvision.transforms.RandomCrop(size=(int(H * ratio), int(W * ratio))),
                torchvision.transforms.Resize(size=(H, W), antialias=True)
            ] + transforms[1:]
            if imagenet_norm:
        ...
        if transforms is not None:
            eval_transforms = [torchvision.transforms.Resize(size=(H, W), antialias=True)]
            if imagenet_norm:
        ...
        print(f"Training transform: {transform}")
        ...
        if three_augment:
            ...
            primary_tfl = [
                torchvision.transforms.RandomCrop(size=(H, W), padding=4, padding_mode='reflect'),
            ]
            secondary_tfl = [
        ...
        feature_map_shape = [H // downsample_ratio, W // downsample_ratio]
    ...
    def forward(self, obs_dict):
        ...
        for key in self.rgb_keys:
            img = obs_dict[key]
            # print(f"Processing RGB key: {key}, input shape: {img.shape}")
            B, C, H, W = img.shape
            assert B == batch_size

            # 直接处理，无需重塑
            if self.training:
                img = self.key_transform_map[key](img)  # img: [B, C, H, W]
            else:
                img = self.key_eval_transform_map[key](img)  # img: [B, C, H, W]
            # print(f"  转移后After transform {key}: {img.shape}")
            raw_feature = self.key_model_map[key](img)  # 模型期望 [B, C, H, W]
            # print(f"  提取的原始特征Extracted raw feature {key}: {raw_feature.shape}")
            fused_feature = None
            if self.fused_model_name != '':
                fused_feature = self.key_fused_model_map[key](img)

            feature = self.aggregate_feature(raw_feature, fused_feature)  # 输出: [B, feature_dim]
            assert len(feature.shape) == 2 and feature.shape[0] == B
            features.append(feature)  # 直接添加，无需 reshape
        ...
        for key in self.low_dim_keys:
            ...
            # 形状检查：确保是 [B, D]
            if len(data.shape) != 2:
                raise ValueError(f"Low-dim input {key} should have shape [B, D], but got {data.shape}")
